################################
# AK_GPORemediation.ps1 - GPO Remediation (Fix/Delete insecure GPOs)
# Extracted from Remediate-BadderBloodGPO.ps1
# Dot-sourced by BadderBloodAnswerKey.ps1
################################
function Invoke-AKGPORemediation {
    <#
        .SYNOPSIS
            Remediates GPO misconfigurations deployed by Invoke-BadderBloodGPO.ps1.
        .PARAMETER Apply
            Actually apply fixes. Without this, runs in report-only mode.
        .PARAMETER DeleteInsecureGPOs
            Delete known insecure GPOs entirely instead of reconfiguring.
        ...
        .PARAMETER DomainDNS
            Domain DNS name.
        .OUTPUTS
            PSCustomObject with RemediationLog and FixCount.
    #>
    ...
    try { Import-Module GroupPolicy -ErrorAction Stop }
        ...
        Write-Status "  GroupPolicy module not available. GPO remediation skipped." "Gray"
        return [PSCustomObject]@{ RemediationLog = @(); FixCount = 0 }
    }

    Write-Host @"
===============================================================================
   BadderBlood GPO Remediation
...
"@ -ForegroundColor $(if($Apply){"Red"}else{"Yellow"})
    ...
    if ($DeleteInsecureGPOs) {
        ...
        # ============================================================================
        # OPTION B: FIX SETTINGS IN-PLACE
        # ============================================================================
            ...
            foreach ($perm in $perms) {
                ...
                if ($permLevel -in @("GpoEdit","GpoEditDeleteModifySecurity","GpoCustom")) {
                    ...
                    try {
                        ...
                        if ($obj.Description) {
                            $isBB = Test-IsBadderBloodObject $obj.Description
                        }
                    } catch {}
