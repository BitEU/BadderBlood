...
.DESCRIPTION
    ...
    Can run standalone or alongside the main BadderBloodAnswerKey.ps1 report.
...
.EXAMPLE
    .\BadderBloodGPO_AnswerKey.ps1
    .\BadderBloodGPO_AnswerKey.ps1 -OutputPath "C:\AnswerKeys\GPO" -ExportCSVs
#>

[CmdletBinding()]
param(
    [string]$OutputPath = ".\BadderBloodGPO_AnswerKey_$(Get-Date -Format 'yyyyMMdd_HHmmss')",
    [switch]$ExportCSVs,
    [switch]$Quiet
)
...
# ============================================================================
# HELPER FUNCTIONS (matching BadderBloodAnswerKey.ps1 format)
# ============================================================================
...
Write-Host @"
===============================================================================
   BadderBlood GPO Answer Key Generator
    ...
    try {
        ...
        foreach ($perm in $perms) {
            ...
            if ($permLevel -in @("GpoEdit", "GpoEditDeleteModifySecurity", "GpoCustom")) {
                ...
                # Check if this trustee is a BadderBlood object
                $isBadderBlood = $false
                try {
                    $obj = Get-ADObject -Filter "SamAccountName -eq '$trustee'" -Properties Description
                    if ($obj.Description) {
                        $isBadderBlood = ($BadderBloodDescPatterns | ForEach-Object { $obj.Description -like $_ }) -contains $true
                    }
                } catch {}

                $bbTag = if ($isBadderBlood) { " [BADDERBLOOD USER]" } else { "" }
