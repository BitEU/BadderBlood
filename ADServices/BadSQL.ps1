...
# svc_sql: BadderBlood does NOT create a real 'svc_sql' AD account.
# Service accounts are named like '1234567SA'. svc_sql exists only as a fictional
# name in BadIIS-generated text files. Create it as a SQL login (not Windows login)
# so the credentials in web_config_backup.xml actually work.
$svcSqlPass = "Sp1ngf!eld_SQL_${ThisYear}#"
Write-Log "Creating svc_sql as SQL login (fictional service account referenced by BadIIS)" "INFO"
Invoke-Sql "IF NOT EXISTS (SELECT 1 FROM sys.server_principals WHERE name = 'svc_sql')
            CREATE LOGIN [svc_sql] WITH PASSWORD = '$svcSqlPass',
            CHECK_POLICY = OFF, CHECK_EXPIRATION = OFF, DEFAULT_DATABASE = [master]"
# svc_sql gets sysadmin to match what BadIIS documents it as
Invoke-Sql "ALTER SERVER ROLE [sysadmin] ADD MEMBER [svc_sql]"
# Set $svcSqlExists so later DB user creation blocks use the SQL login path instead of Windows login path
$svcSqlExists = $true
$svcSqlLoginName = 'svc_sql'  # SQL login, not domain\svc_sql
...
foreach ($db in $databases) {
    Invoke-Sql @"
USE [$db];
IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = 'svc_sql')
    CREATE USER [svc_sql] FOR LOGIN [svc_sql];
ALTER ROLE [db_owner] ADD MEMBER [svc_sql];
"@
...
if ($ADMode -and $ADUsers.Count -gt 0) {
    ...
    # BadderBlood uses 'HRE' as the department acronym (not 'HR').
    # Users have no EmailAddress set, so construct address from SamAccountName + domain.
    $hrUsers = $ADUsers | Where-Object { $_.Department -eq 'HRE' -or $_.Department -match 'Human Rel' } | Select-Object -First 5
    $hrOfficers = if ($hrUsers.Count -gt 0) {
        $hrUsers | ForEach-Object { "$($_.SamAccountName)@$DomainDNS" }
    } else {
...
SERVICE ACCOUNTS:
  svc_sql            - SQL login service account (sysadmin, db_owner on all DBs)
...
svc_sql                     SQL Login       YES       Kerberoastable. SPN: MSSQLSvc/$PDC`:1433
